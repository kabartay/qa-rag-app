...
class QAGenerator:
    ...
    def generate_qa_pairs(self, text: str, num_pairs: int = 10, difficulty: str = "mixed") -> List[Dict]:
...
def main():
    ...
    # Generation controls
    st.header("2️⃣ Generate Q&A Pairs")
    ...
    with col3:
        if st.button("🗑️ Clear All", disabled=not st.session_state.qa_pairs):
            st.session_state.qa_pairs = []
            st.rerun()
    
    # Display results
    if st.session_state.qa_pairs:
        st.header("3️⃣ Generated Q&A Pairs")
        ...
        types = [p.get('type', 'unknown') for p in pairs]
